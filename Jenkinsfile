    ...
    stages {
            ...
            stage("Dockerize") {
                steps {
                    ...
                    sh 'docker build --file=Dockerfile-gateway --tag="$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$REPO_NAME:$(git rev-parse HEAD)" \
            ...
            stage("Push") {
                steps {
                    ...
                    dir('target'){
                        sh 'curl -X "PUT" -u "$JFROG_USER:$JFROG_PW" --upload-file "aline-gateway-0.0.1-SNAPSHOT.jar" "http://$JFROG_HOST/artifactory/microservices-jar-local/gateway-microservice-0.0.1.jar"'
                    }
